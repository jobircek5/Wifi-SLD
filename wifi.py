...
class RunConfiguration:
    ...
    def __init__(self):
        ...
        # True means the program must find a valid handshake in order for SLD-7x-wife to recognize a handshake.
        ...
        self.IFACE_TO_TAKE_DOWN = ''  # Interface that SLD-7x-wife puts into monitor mode
    ...
    def ConfirmRunningAsRoot(self):
        if os.getuid() != 0:
            print R + ' [!]' + O + ' ERROR:' + G + ' SLD-7x-wife' + O + ' must be run as ' + R + 'root' + W
            print R + ' [!]' + O + ' login as root (' + W + 'su root' + O + ') or try ' + W + 'sudo ./SLD-7x-wife.py' + W
            exit(1)

    def ConfirmCorrectPlatform(self):
        if not os.uname()[0].startswith("Linux") and not 'Darwin' in os.uname()[0]:  # OSX support, 'cause why not?
            print O + ' [!]' + R + ' WARNING:' + G + ' SLD-7x-wife' + W + ' must be run on ' + O + 'linux' + W
            exit(1)

    def CreateTempFolder(self):
        from tempfile import mkdtemp

        self.temp = mkdtemp(prefix='SLD-7x-wife')
    ...
    def exit_gracefully(self, code=0):
        ...
        mac_change_back()
        print GR + " [+]" + W + " quitting"  # SLD-7x-wife will now exit"
        print ''
        # GTFO
        exit(code)
...
class RunEngine:
    ...
    def initial_check(self):
        ...
        if not program_exists('iwconfig'):
            print R + ' [!]' + O + ' SLD-7x-wife requires the program %s\n' % (R + 'iwconfig' + W)
            self.RUN_CONFIG.exit_gracefully(1)
    ...
    def scan(self, channel=0, iface='', tried_rtl8187_fix=False):
        ...
        """
        airodump_file_prefix = os.path.join(self.RUN_CONFIG.temp, 'SLD-7x-wife')
        ...
        try:
            ...
            while True:
                ...
                if not os.path.exists(csv_file) and time.time() - time_started > 1.0:
                    ...
                    print R + ' [!]' + O + ' SLD-7x-wife is unable to generate airodump-ng output files' + W
                    print R + ' [!]' + O + ' you may want to disconnect/reconnect your wifi device' + W
                    self.RUN_CONFIG.exit_gracefully(1)
...
def banner(RUN_CONFIG):
    ...
    print ''
    print G + "  .;'                     `;,    "
    print G + " .;'  ,;'             `;,  `;,   " + W + "SLD-7x-wife v2.0 (r" + str(RUN_CONFIG.REVISION) + ")"
    print G + ".;'  ,;'  ,;'     `;,  `;,  `;,  "
    print G + "::   ::   :   " + GR + "( )" + G + "   :   ::   ::  " + GR + "automated wireless auditor"
    print G + "':.  ':.  ':. " + GR + "/_\\" + G + " ,:'  ,:'  ,:'  "
    print G + " ':.  ':.    " + GR + "/___\\" + G + "    ,:'  ,:'   " + G + "try the new version:"
    print G + "  ':.       " + GR + "/_____\\" + G + "      ,:'     " + C + "https://github.com/derv82/SLD-7x-wife2"
    print G + "           " + GR + "/       \\" + G + "             "
    print W
...
def help():
    ...
    print head + '\n   EXAMPLE' + W
    print sw + '\t./SLD-7x-wife.py ' + W + '-wps -wep -c 6 -pps 600' + W
    print ''
